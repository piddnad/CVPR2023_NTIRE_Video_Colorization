...
class user_dataset(Dataset):
    def __init__(self, data_dir, interval=1, mode='val'):
        self.data_dir = data_dir

        filenames = np.load(f'ntire23_scripts/assets/{mode}_filenames.npy')

        if interval > 1:
            n = len(filenames)
            self.filenames = filenames[0:n:interval]
        else:
...
def calculate_fid(pred_dir, interval=1, mode='val'):
    ...
    dataset = user_dataset(pred_dir, interval=interval, mode=mode)
    print(len(dataset))
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=False)
    fake_acts_set, acts_set = [], []
    with torch.no_grad():
        for i, batch in enumerate(tqdm(dataloader)):
            pred = batch.to(device)
        ...
        real_mu = np.load(f'ntire23_scripts/assets/{mode}_gt_mu.npy')
        real_sigma = np.load(f'ntire23_scripts/assets/{mode}_gt_sigma.npy')
